import { SectionCard } from '@/components/cards/section-card'
import { mainNavigation } from '@/lib/navigation'

export const metadata = { title: 'Студенческая жизнь' }

export default function StudentLifePage() {
  const section = mainNavigation.find((item) => item.href === '/student-life')
  const children = section?.children ?? []

  return (
    <div className="container mx-auto px-4 py-12 space-y-10">
      <header className="space-y-4">
        <h1 className="font-heading text-4xl md:text-5xl text-dark">{section?.title}</h1>
        <p className="text-gray max-w-2xl">{section?.description}</p>
      </header>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 auto-rows-[1fr]">
        {children.map((child) => {
          let iconKey = child.href.split('/').pop();
          if (iconKey === 'events') iconKey = 'events';
          return (
            <SectionCard
              key={child.href}
              href={child.href}
              title={child.title}
              description={child.description}
              iconKey={iconKey}
            />
          )
        })}
      </div>
    </div>
  )
} 